...
- Use the architecture convention for the source map, the source dependency convention for import direction, and the runtime wiring convention for provider binding.
...
- Put orchestration in the application layer when code decides what to load, authorize, call, and save to execute a use case.
- Put implementation in the infrastructure layer when code decides how to call an external process, persist, publish, or use a technical library.
- Application services load needed objects, call domain methods or domain services, and save changes. They should not implement domain judgments directly.
- Infrastructure adapters implement external process, file system, SDK, persistence, and other technical details behind domain or application contracts.

### Value Object Raw Value Access

- Use `unpack()` to read the raw value from a value object when the base value-object contract provides it.
...
- If a project-specific value object intentionally exposes named readonly fields, keep that API focused on the domain meaning, not on mirroring a request DTO.
...
- `find` looks up one aggregate or read model by a unique lookup and returns `undefined` or `null` when it is absent. Keep the absence type consistent inside a repository contract.
- `find` should express lookup meaning through object parameter field names. Example: `find({ id })`.
...
- Ask the object to answer a domain question or perform a domain action, such as `isExpired`, `touch`, or `attachClaudeSession`, instead of pulling fields out and deciding externally.
...
- Use local, non-exported types when a shape is reused inside one aggregate, would make signatures noisy, or represents internal restore or mapping details.
...
- Check whether a domain object is expressing business behavior instead of acting as a storage row or request DTO.
...
- Check whether an application service is invoking domain behavior instead of extracting state and making domain decisions externally.
