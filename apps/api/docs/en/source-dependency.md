...
- The enforceable source dependency rules live in `apps/api/dependency-cruiser/rules/source-dependency.cjs` and `apps/api/dependency-cruiser/rules/runtime-wiring.cjs`.
...
| `kernels` | `core` and same kernel directory | bounded context implementations, `platform`, framework code, and outer layers |
| `domain` | `core`, `kernels/domain`, and same-context domain code | `application`, `infrastructure`, `presentation`, `platform`, NestJS, database, HTTP, and SDK code |
| `application` | `core`, same-context `domain`, `kernels/application`, same-context application code, same-context `*.di-tokens.ts` files, and narrow NestJS DI APIs used only for provider construction | infrastructure implementations, presentation DTOs, non-DI framework runtime APIs, and platform concrete types |
...
| `presentation` | `core`, `application`, `kernels/presentation`, frameworks, or protocol libraries when handling external protocols | domain internals, infrastructure implementations, process adapters, and SDK adapters |
...
- Production imports into kernel directories and context domain code should use their public surfaces.
...
- Domain code MUST NOT know application, infrastructure, presentation, framework, database, HTTP, process, or SDK details.
...
- Aside from the narrow DI metadata and same-context provider tokens allowed above, application core may depend on `core`, same-context domain code, same-context application code, and `kernels/application`.

### Infrastructure Layer

- The infrastructure layer implements technical adapters.
- Use it for external process, file system, SDK, persistence, message broker, and other technical integration code.
- Infrastructure code implements application-owned ports or domain/application contracts.
- Adapter code may wrap technology-specific errors, such as CLI process, HTTP client, or SDK errors, in regular `Error` objects with `cause` when adding adapter context.
...
- Use it for controllers, request DTOs, response DTOs, protocol mappers, and HTTP error mappers.
- Presentation code calls application use cases or services.
...
Run source dependency checks from the repository root:
