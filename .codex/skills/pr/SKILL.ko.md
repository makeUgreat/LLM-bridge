...
1. `git status --short`, 최근 커밋, 관련 diff 또는 브랜치 비교를 확인합니다. 구조화된 fact pack이 PR 단위 결정을 더 빠르거나 덜 오류 나게 만든다면 `scripts/inspect-pr-context.sh`를 사용합니다.
...
11. blocker가 없는 한 준비된 제목, 본문, base branch, merge-order note로 명확한 단일 PR 또는 stacked PR 묶음을 추가 확인 없이 push하고 draft PR을 생성합니다.